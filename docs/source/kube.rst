Scaling [testmytss2-3f10] With Kubernetes
===========================
...
Based on your TML solution [testmytss2-3f10] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`testmytss2-3f10.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-testmytss2-3f10.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingresstestmytss2-3f10.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f testmytss2-3f10.yml

testmytss2-3f10.yml
------------------------

.. important::
   Copy and Paste this YAML file: testmytss2-3f10.yml - and save it locally.

   These YML files can also be found on GitHub at: https://github.com/smaurice101/raspberrypitss/tree/main/tml-airflow/dags/tml-solutions/testmytss2-3f10/ymls/testmytss2-3f10
...
.. code-block:: YAML
     ...
     spec:
       ...
       template:
         ...
         spec:
           containers:
           - name: testmytss2-3f10
             image: maadsdocker/testmytss2-3f10-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: PROJECTNAME
               value: 'testmytss2-3f10'                              
             - name: SOLUTIONNAME
               value: 'testmytss2-3f10'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_dag-cybersecurity-rtms-main-3f10'
             - name: GITUSERNAME
               value: 'smaurice101'
             - name: GITREPOURL
   ...
   ---
     ...
     kind: Service
     metadata:
       name: testmytss2-3f10-visualization-service
       labels:
         app: testmytss2-3f10-visualization-service
     spec:
       type: ClusterIP
       ports:
...
.. code-block::
   ...
   kubectl port-forward deployment/testmytss2-3f10 6060:6060

After you forward the ports then copy/paste the viusalization URL below and run your dashboard.

.. code-block::

   http://localhost:6060/dashboard-rtms.html?topic=iot-preprocess,iot-preprocess2&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-testmytss2-3f10.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-testmytss2-3f10.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-testmytss2-3f10.yml
...
.. code-block::
   ...
   http://tml.tss/viz/dashboard-rtms.html?topic=iot-preprocess,iot-preprocess2&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
